...
def isPositionAvailable(row, col, maze):  # row and col of the place the User wishes to move to
    if row < 0 or col < 0:  # Checks if User is attempting to go out of bounds
        return False
    if maze[row][col] == '1':  # Checks if User is attempting to go through walls
        return False
...
def playGame(maze, row, col):  # row and col given here is the starting position
    ...
    for command in commands:
        ...
        if command == "R":
            col = col + 1
            isPositionAvailable(row, col, maze)
        elif command == "L":
            col = col - 1
            isPositionAvailable(row, col, maze)
        elif command == "U":
